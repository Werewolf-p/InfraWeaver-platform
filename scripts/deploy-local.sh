...
#
# WHAT THIS DOES (equivalent to full-redeploy.yml GitHub Actions workflow):
...
REQUIRED_VARS=(PROXMOX_API_TOKEN DEPLOYER_SSH_KEY CLOUDFLARE_API_TOKEN SMTP_USERNAME SMTP_PASSWORD)
MISSING=()
for v in "${REQUIRED_VARS[@]}"; do
  val="${!v:-}"
  if [[ -z "$val" || "$val" == *"<your"* || "$val" == *"xxxx"* ]]; then
    MISSING+=("$v")
  fi
done
...
PVE_IP=$(grep 'proxmox_host:' "envs/$ENV_NAME/cluster.yaml" 2>/dev/null | head -1 | sed 's/.*: *"\(.*\)"/\1/' | xargs || echo "10.25.0.3")
...
[[ -n "${RUNNER_REGISTRATION_TOKEN:-}" ]] && export TF_VAR_github_runner_token="$RUNNER_REGISTRATION_TOKEN"
...
# Build homelab-ansible Docker image if GITHUB_RUNNER_TOKEN is set and image is missing
SKIP_RUNNERS=false
if [[ -z "${TF_VAR_github_runner_token:-}" || "${TF_VAR_github_runner_token:-}" == "placeholder"* ]]; then
  log "==> GITHUB_RUNNER_TOKEN not configured — skipping GitHub runner provisioning"
  SKIP_RUNNERS=true
else
  log "==> Building homelab-ansible Docker image (needed for GitHub runner provisioning)..."
  if docker images -q homelab-ansible:latest 2>/dev/null | grep -q .; then
    ...
    log "⚠️  homelab-ansible image build failed — skipping GitHub runner provisioning"
    SKIP_RUNNERS=true
  fi
fi
...
if [[ "$SKIP_RUNNERS" == "true" ]]; then
  # Write a temp override that disables github_runners
  RUNNER_OVERRIDE=$(mktemp /tmp/iw-no-runners-XXXXXX.tfvars)
  echo 'github_runners = {}' > "$RUNNER_OVERRIDE"
