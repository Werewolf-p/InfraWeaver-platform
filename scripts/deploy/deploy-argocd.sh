...
kubectl --kubeconfig "$KB" delete ingress argocd-server-platform -n argocd 2>/dev/null \
  && echo "🧹 Removed stale argocd-server-platform ingress" \
  || echo "  argocd-server-platform ingress not present (expected)"
