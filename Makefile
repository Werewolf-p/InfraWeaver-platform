...
push-secrets: ## Optional: sync .env → GitHub Secrets using gh CLI
	@bash .github/optional/scripts/push-secrets-to-github.sh
...
lint-actions: ## Lint GitHub Actions workflows with actionlint (optional)
	@if [ -d .github/workflows ] && ls .github/workflows/*.yml >/dev/null 2>&1; then \
		...
		echo "==> No GitHub Actions workflows present — skipping actionlint"; \
	fi
