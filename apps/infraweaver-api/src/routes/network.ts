...
networkRoute.get('/topology', async (c) => {
  ...
  try {
    ...
    type TopoNode = { id: string; type: string; name: string; namespace: string; status: string };
    type TopoEdge = { source: string; target: string };
    const nodes: TopoNode[] = [{ id: 'traefik', type: 'ingress-controller', name: 'Traefik', namespace: 'traefik', status: 'healthy' }];
    const edges: TopoEdge[] = [];

    if (ingressRoutesResp.status === 'fulfilled') {
      for (const ir of ((ingressRoutesResp.value as { items?: unknown[] }).items ?? [])) {
        const r = ir as { metadata?: { name?: string; namespace?: string }; spec?: { routes?: Array<{ services?: Array<{ name?: string; namespace?: string }> }> } };
    ...
    if (svcsResp.status === 'fulfilled') {
      for (const svc of ((svcsResp.value as { items?: unknown[] }).items ?? [])) {
        const s = svc as { metadata?: { name?: string; namespace?: string }; spec?: { selector?: Record<string, string>; type?: string } };
    ...
    if (podsResp.status === 'fulfilled' && svcsResp.status === 'fulfilled') {
      ...
      for (const pod of ((podsResp.value as { items?: unknown[] }).items ?? [])) {
        const p = pod as { metadata?: { name?: string; namespace?: string; labels?: Record<string, string> }; status?: { phase?: string; conditions?: Array<{ type?: string; status?: string }> } };
        ...
        for (const svc of svcs) {
          const s = svc as { metadata?: { name?: string; namespace?: string }; spec?: { selector?: Record<string, string> } };
