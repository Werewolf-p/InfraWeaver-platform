...
let _broadcastFn: ((mode: ApiMode) => void) | null = null;
...
export async function getMode(): Promise<ApiMode> {
...
export async function setMode(mode: ApiMode): Promise<void> {
  const coreApi = getCoreApi();
...
export function registerBroadcastFn(fn: (mode: ApiMode) => void) {
  _broadcastFn = fn;
}

function broadcastModeChange(mode: ApiMode) {
  _broadcastFn?.(mode);
}
