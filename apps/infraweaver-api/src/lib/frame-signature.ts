...
/**
 * Signed-frame wire contract shared with the cluster agent.
 *
 * MUST stay byte-for-byte in sync with
 * `apps/infraweaver-node/src/types/index.ts` (payload construction) and
 * `apps/infraweaver-node/src/lib/crypto.ts` (SHA256 ECDSA, base64 signature).
 * A drift here silently disconnects every live agent (C5).
 */
