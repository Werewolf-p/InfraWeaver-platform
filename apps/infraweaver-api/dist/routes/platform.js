...
async function getLatestRelease() {
    try {
        return await ghFetch(`/repos/${GITHUB_REPO}/releases/latest`);
    }
    catch {
        return null;
    }
...
async function getLatestCommit() {
    try {
        const commits = await ghFetch(`/repos/${GITHUB_REPO}/commits?sha=main&per_page=1`);
...
async function getChangelogSince(since) {
    try {
        // Try release-based changelog first
        const releases = await ghFetch(`/repos/${GITHUB_REPO}/releases?per_page=10`);
        ...
        const commits = await ghFetch(`/repos/${GITHUB_REPO}/commits?sha=main&per_page=15`);
...
// ── ArgoCD helpers ────────────────────────────────────────────────────────────
async function argocdHardRefresh(appName) {
    try {
...
function rewriteImageTag(yaml, appName, newTag) {
    // Matches: "image: <any-registry>/<any-path>/infraweaver-{app}:<old-tag>"
    const re = new RegExp(`(\\bimage:\\s*)[^\\s]*/infraweaver-${appName}:[^\\s]+`, "gm");
    if (!re.test(yaml))
        return null;
    const updated = yaml.replace(new RegExp(`(\\bimage:\\s*)[^\\s]*/infraweaver-${appName}:[^\\s]+`, "gm"), `$1ghcr.io/${GHCR_ORG}/infraweaver-${appName}:${newTag}`);
    // Also update APP_VERSION env var if present
    return updated.replace(/(name:\s*APP_VERSION\s*\n\s*value:\s*")[^"]+(")/m, `$1${newTag}$2`);
}
// ── Routes ────────────────────────────────────────────────────────────────────
...
route.post("/update", async (c) => {
    ...
    for (const [appName, filePath] of Object.entries(DEPLOYMENT_MANIFESTS)) {
        try {
            ...
            await onedevPutFile(filePath, updated, `ci: update infraweaver-${appName} to ${targetVersion}`);
            results[appName] = { ok: true };
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
    ...
    const refreshResults = await Promise.allSettled(ARGOCD_APPS.map((app) => argocdHardRefresh(app)));
...
route.post("/trigger-ci", async (c) => {
    ...
    try {
        ...
        const runs = await ghFetch(`/repos/${GITHUB_REPO}/actions/workflows/release.yml/runs?per_page=1`);
...
route.get("/workflow/:runId", async (c) => {
    ...
    try {
        const run = await ghFetch(`/repos/${GITHUB_REPO}/actions/runs/${runId}`);
        return c.json({
            ok: true,
            runId: run.id,
