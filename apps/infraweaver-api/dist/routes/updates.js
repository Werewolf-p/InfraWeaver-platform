...
import { z } from 'zod';
import { VERSION_SOURCES } from '../config/version-sources.js';
import { getCluster } from '../lib/cluster-registry.js';
import { githubGetFile, githubGetTree, githubPutFile, isOnedev, onedevGetFile, onedevGetTreeAndFiles, onedevPutFile, } from '../lib/git-provider.js';
...
function getCurrentVersion(liveApp, manifest) {
...
function getSyncStatus(liveApp) {
...
function extractHelmChartVersions(indexYaml, chartName) {
    const versions = [];
    const lines = indexYaml.split(/\r?\n/);
    let afterEntries = false;
    let chartHeaderRe = null;
    let itemVersionRe = null;
...
async function fetchHelmVersions(source) {
...
async function fetchDockerVersions(source) {
    const { owner, repository } = parseDockerImage(source.image);
    const response = await fetchJson(`https://hub.docker.com/v2/repositories/${owner}/${repository}/tags?page_size=25&ordering=last_updated`);
    return {
        source: 'docker',
        versions: uniqueSortedVersions((response.results ?? []).map((tag) => tag.name ?? '')).slice(0, 15),
    };
}
async function fetchGhcrVersions(source) {
...
async function getAvailableVersions(source) {
    if (!source) {
        return {
            source: 'unknown',
            versions: [],
...
function parseManifestEntry(filePath, content) {
...
async function collectApplicationManifests() {
    const now = Date.now();
    ...
    if (isOnedev()) {
        ...
        rawEntries = (await Promise.all(treeItems.map(async ({ path: p }) => {
...
async function getArgoConfig(clusterId) {
    if (clusterId === 'local') {
        return {
...
async function listApplicationCrds(clusterId) {
    try {
...
async function fetchArgoApplications(clusterId) {
...
function replaceTargetRevision(content, version) {
    ...
    const updatedContent = content.replace(pattern, (_, prefix, quote, _current, closingQuote, suffix) => {
