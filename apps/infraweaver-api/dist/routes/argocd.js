...
function getCachedOrMock(clusterId) {
...
async function fetchApps(clusterId) {
...
export const argocdRoute = new Hono();
...
argocdRoute.post('/apps/bulk', async (c) => {
    ...
    const { server, token } = await getArgoConfig(user.clusterId);
    if (!server || !token)
        return c.json({ ok: true, mock: true, results: apps.map((name) => ({ name, ok: true })) });
