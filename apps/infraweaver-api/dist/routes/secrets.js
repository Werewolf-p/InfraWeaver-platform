...
secretsRoute.get('/', async (c) => {
    ...
    try {
        ...
        const externalSecrets = ((externalSecretsResponse.items ?? [])).map((item) => {
            const es = item;
            return { name: es.metadata?.name ?? '', namespace: es.metadata?.namespace ?? 'default', targetSecret: es.spec?.target?.name ?? es.metadata?.name ?? '' };
        });
        const managedSecrets = new Map(externalSecrets.map((es) => [`${es.namespace}/${es.targetSecret}`, `${es.namespace}/${es.name}`]));
...
secretsRoute.delete('/:namespace/:name', async (c) => {
    ...
    if (!hasPermission(user, 'cluster:admin'))
        return c.json({ error: 'Forbidden' }, 403);
    if (user.clusterId === 'all')
        return c.json({ error: 'Select a specific cluster before performing this action' }, 400);
