...
export async function githubPutFile(filePath, content, message, sha) {
...
async function withWorktree(worker) {
    if (!ONEDEV_TOKEN)
        throw new Error('ONEDEV_TOKEN is not set');
...
async function walkDir(fsDir, relDir, filter) {
    const results = [];
...
export async function onedevGetTreeAndFiles() {
...
export async function onedevGetFile(filePath) {
    return withWorktree(async (dir, git) => {
        try {
...
export async function onedevPutFile(filePath, content, message) {
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            if (attempt < 2 && isPushRejection(err))
                continue;
            throw err;
        }
    }
