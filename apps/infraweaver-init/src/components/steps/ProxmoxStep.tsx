...
import { CheckCircle2, Database, Eye, EyeOff, LoaderCircle, Lock, Server, ShieldCheck, Sparkles, Waypoints } from 'lucide-react'
...
export function ProxmoxStep() {
  ...
  const handleSetupUser = async () => {
    ...
    if (!host) { setSetupResult({ ok: false, message: 'Enter Proxmox host / IP first' }); return }
    if (!adminUser.trim()) { setSetupResult({ ok: false, message: 'Enter admin username' }); return }
    if (!adminPass) { setSetupResult({ ok: false, message: 'Enter admin password' }); return }
    ...
    try {
      ...
      if (result.ok && result.token) {
        ...
        setAdminPass('')  // clear password from state immediately
  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const firstDs = result.datastores?.[0] ?? data.TALOS_DATASTORE
        setFields({
          PROXMOX_NODE_NAME: firstNode,
          TALOS_DATASTORE: firstDs,
          ...
          PVE_NODES: result.pve_nodes_str ?? data.PVE_NODES,
          NODE_1_PVE_NODE: firstNode,
          NODE_2_PVE_NODE: firstNode,
          NODE_3_PVE_NODE: firstNode,
          NODE_1_DATASTORE: (result.datastores_by_node?.[firstNode]?.[0] ?? firstDs),
          NODE_2_DATASTORE: (result.datastores_by_node?.[firstNode]?.[0] ?? firstDs),
          NODE_3_DATASTORE: (result.datastores_by_node?.[firstNode]?.[0] ?? firstDs),
        })
        applyHealthyStatus(true)
      }
    } finally {
          ...
          {pveMode === 'setup' && (
            <motion.div key="setup" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-6 space-y-4">
              <div className="flex items-start gap-3 rounded-2xl border border-[rgba(87,163,0,0.25)] bg-[rgba(87,163,0,0.07)] p-4 text-sm text-[var(--az-text-secondary)]">
                ...
                <span>
                  <strong className="text-white">Credentials are never stored.</strong>{' '}
                  They are used once to create the <code className="text-white">infraweaver@pve</code> API user with minimal permissions. Only the resulting API token is saved.
                </span>
              </div>
              ...
              <FormField label="Admin password" htmlFor="pve-admin-pass">
                <div className="relative">
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') void handleSetupUser() }}
                    placeholder="••••••••"
                    className={`${controlClassName} pr-12`}
                    autoComplete="current-password"
                  />
                  <button type="button" onClick={() => setShowPass((v) => !v)} className="absolute inset-y-0 right-0 inline-flex w-12 items-center justify-center text-[var(--az-text-secondary)] hover:text-white">
              ...
              >
                <div className="relative">
                  <input
                    ...
                    className={`${controlClassName} pr-12`}
                  />
                  <button type="button" onClick={() => setShowToken((v) => !v)} className="absolute inset-y-0 right-0 inline-flex w-12 items-center justify-center text-[var(--az-text-secondary)] hover:text-white">
        ...
        <GlassCard className="p-6">
          <motion.div variants={fadeUpItem} className="mb-5 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-[var(--az-text-secondary)]">Auto-fills node, storage, and VMIDs in the cluster step.</div>
            </div>
          </motion.div>
          ...
          <div className="space-y-4">
            <motion.div variants={fadeUpItem} className="rounded-2xl border border-white/8 bg-black/20 p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {(proxmoxDiscovery?.all_nodes?.length ? proxmoxDiscovery.all_nodes : [data.PROXMOX_NODE_NAME || 'pve']).map((node) => (
                  <span key={node} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white">{node}</span>
                ))}
              </div>
            </motion.div>
