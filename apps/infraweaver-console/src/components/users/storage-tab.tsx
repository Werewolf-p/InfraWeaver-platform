...
interface WizardState {
  username: string;
  /** Any registered provider id (built-in or dynamically added), or "" when unset. */
  provider: string;
  share: string;
  subfolder: string;
...
function generateYaml(state: WizardState, host: string): string {
  const scName = `smb-${state.username}-${state.share.toLowerCase()}`;
  // Matches the server manifest generator's per-share/per-access secret naming
  // (`nas-<share>-<ro|rw>`) so the preview reflects what actually gets committed.
  const accessSuffix = state.access === "readonly" ? "ro" : "rw";
  const secretName = `nas-${state.share.toLowerCase().replace(/[^a-z0-9-]/g, "-")}-${accessSuffix}`;
  return `---
...
function AssignWizard({
  ...
  const selectedHost = providers.find((p) => p.id === state.provider)?.host ?? "";
  ...
  const copyYaml = () => {
    navigator.clipboard.writeText(generateYaml(state, selectedHost)).then(() => {
      setCopied(true);
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-modal flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm"
      onClick={onClose}
    >
      <motion.div
                ...
                <div className="space-y-3">
                  {providers.map(p => {
                    ...
                    return (
                      <button
                        key={p.id}
                        onClick={() => p.reachable && updateState({ provider: p.id, share: "", subfolder: state.username })}
              ...
              <motion.div key="step5" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-4">
                ...
                <div>
                  ...
                  <pre className="bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-white/10 rounded-lg p-4 text-xs text-slate-700 dark:text-slate-300 overflow-x-auto max-h-56 leading-relaxed font-mono whitespace-pre">
                    {generateYaml(state, selectedHost)}
                  </pre>
                </div>
              </motion.div>
            )}
