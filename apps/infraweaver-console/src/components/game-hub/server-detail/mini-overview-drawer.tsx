  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-4 right-4 z-40 inline-flex items-center gap-2 rounded-full border border-[#2a2a2a] bg-[#111] px-4 py-2 text-sm text-[#f2f2f2] shadow-[0_10px_30px_rgba(0,0,0,0.45)] transition-colors hover:bg-[#1a1a1a]"
      >
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              exit={{ y: 24, opacity: 0 }}
              className="fixed inset-x-0 bottom-0 z-50 mx-auto max-w-4xl rounded-t-2xl border border-[#2a2a2a] bg-[#111] p-4 shadow-2xl"
            >
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-[#f2f2f2]">Server overview</p>
                  <p className="text-xs text-[#888]">All accessible game servers</p>
                </div>
                <button
                  onClick={() => setOpen(false)}
                  className="rounded-lg border border-[#2a2a2a] bg-[#0d0d0d] p-2 text-[#888] hover:text-white"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="max-h-[55vh] space-y-2 overflow-y-auto pr-1">
                {servers.length === 0 ? (
                  <p className="rounded-xl border border-[#2a2a2a] bg-[#0d0d0d] px-4 py-3 text-sm text-[#888]">
                  ...
                  servers.map((server) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "w-full rounded-xl border border-[#2a2a2a] bg-[#0d0d0d] px-4 py-3 text-left transition-colors hover:bg-[#151515]",
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <span className="truncate text-sm font-medium text-[#f2f2f2]">{server.name}</span>
                            </div>
                            <p className="mt-1 text-[11px] uppercase tracking-wide text-[#666]">{server.status}</p>
                          </div>
