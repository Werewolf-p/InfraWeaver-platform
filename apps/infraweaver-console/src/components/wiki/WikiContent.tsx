...
function Heading({ level, children }: { level: 2 | 3 | 4; children: React.ReactNode }) {
  ...
  const className =
    level === 2
      ? "mt-10 scroll-mt-28 text-2xl font-semibold text-white"
      : level === 3
        ? "mt-8 scroll-mt-28 text-xl font-semibold text-white"
  ...
  return React.createElement(
    ...
    <span className="group inline-flex items-center gap-2">
      ...
      <a
        href={`#${id}`}
        className="rounded-md border border-transparent p-1 text-slate-500 opacity-0 transition hover:border-white/10 hover:bg-white/5 hover:text-blue-300 group-hover:opacity-100"
...
function CodeBlock({ className, children, inline }: { className?: string; children?: React.ReactNode; inline?: boolean }) {
  ...
  return (
    <div className="my-6 overflow-hidden rounded-2xl border border-white/10 bg-[#0b0f14] shadow-sm">
      <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-4 py-2 text-xs uppercase tracking-[0.18em] text-slate-400">
        <span>{language}</span>
        <button
          type="button"
          onClick={() => void copyToClipboard()}
          className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[11px] font-medium normal-case tracking-normal text-slate-300 transition hover:bg-white/[0.08] hover:text-white"
        >
...
function BlockQuote({ children }: { children: React.ReactNode }) {
  ...
  const styles = isWarning
    ...
    : isNote
      ...
      : "border-slate-500/30 bg-white/[0.03] text-slate-100";
    ...
    <div className="text-[15px] leading-7 text-slate-200">
      <ReactMarkdown
        ...
        components={{
          ...
          p: ({ children }) => <p className="mt-4 text-slate-200">{children}</p>,
          ...
          li: ({ children }) => <li className="pl-1 text-slate-200">{children}</li>,
