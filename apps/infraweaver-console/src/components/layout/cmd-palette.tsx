      ...
      if (podsResult.status === "fulfilled") {
        ...
        nextItems.push(
          ...pods.slice(0, 16).map((pod) => ({
            ...
            href: `/logs?namespace=${encodeURIComponent(pod.namespace ?? "default")}&pod=${encodeURIComponent(pod.name ?? "")}`,
        ...
        nextItems.push(
          ...namespaces.map((namespace) => ({
            ...
            subtitle: "Browse pods in logs",
            href: `/logs?namespace=${encodeURIComponent(namespace)}`,
            category: "Pods" as const,
            icon: Server,
            keywords: [namespace],
          }))
