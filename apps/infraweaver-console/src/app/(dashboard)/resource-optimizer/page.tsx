...
export default function ResourceOptimizerPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Activity className="w-5 h-5 text-slate-500 dark:text-slate-400" />Resource Optimizer</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400">Request vs limit recommendations</p>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          { label: "Pods Analyzed", value: recs.length, color: "text-gray-900 dark:text-white" },
      ...
      <div className="bg-slate-100 dark:bg-slate-900/60 border border-gray-200 dark:border-white/10 rounded-xl backdrop-blur-sm overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-gray-200 dark:border-white/10">
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Pod / Container</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Namespace</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">CPU Request</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">CPU Recommended</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Mem Request</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Mem Recommended</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Status</th>
          </tr></thead>
          <tbody>
            {recs.flatMap(r => r.containers.map(c => (
              <tr key={`${r.pod}/${c.name}`} className="border-b border-gray-200 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                <td className="px-4 py-3">
                  <p className="text-sm text-gray-900 dark:text-white font-medium">{r.pod}</p>
                  <p className="text-xs text-slate-500">{c.name}</p>
                </td>
                <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{r.namespace}</td>
                <td className="px-4 py-3 text-xs text-center text-slate-700 dark:text-slate-300 font-mono">{c.requestCpu}</td>
                <td className="px-4 py-3 text-xs text-center text-indigo-300 font-mono">{c.recommendedCpu}</td>
                <td className="px-4 py-3 text-xs text-center text-slate-700 dark:text-slate-300 font-mono">{c.requestMemory}</td>
