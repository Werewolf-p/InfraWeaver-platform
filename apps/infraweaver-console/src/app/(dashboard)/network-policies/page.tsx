    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">Kubernetes NetworkPolicy resources</p>
        </div>
      ...
      <div className="bg-slate-900/60 border border-white/10 rounded-xl backdrop-blur-sm overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-white/10">
            ...
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-400">Ingress Rules</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-400">Egress Rules</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400">Types</th>
            ...
            {filtered.map(p => (
              <>
                <tr key={p.name} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-center text-slate-300">{p.ingressRules}</td>
                  <td className="px-4 py-3 text-sm text-center text-slate-300">{p.egressRules}</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => setShowYaml(showYaml === p.name ? null : p.name)} className="text-xs text-slate-400 hover:text-white transition-colors">
                      {showYaml === p.name ? "Hide" : "YAML"}
                    </button>
                  </td>
                </tr>
                {showYaml === p.name && (
                  <tr key={`${p.name}-yaml`} className="border-b border-white/5">
