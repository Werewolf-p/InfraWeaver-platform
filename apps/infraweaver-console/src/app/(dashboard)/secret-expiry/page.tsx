  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">TLS certificate and secret expiration monitoring</p>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          { label: "Total TLS Secrets", value: secrets.length, color: "text-gray-900 dark:text-white" },
      ...
      <div className="bg-slate-100 dark:bg-slate-900/60 border border-gray-200 dark:border-white/10 rounded-xl backdrop-blur-sm overflow-hidden">
        <table className="w-full">
          <thead><tr className="border-b border-gray-200 dark:border-white/10">
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Secret</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Namespace</th>
            <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Expires</th>
            <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400">Days Left</th>
          </tr></thead>
          <tbody>
            {secrets.map(s => (
              <tr key={`${s.namespace}/${s.name}`} className="border-b border-gray-200 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white font-medium">{s.name}</td>
                <td className="px-4 py-3 text-sm text-slate-500 dark:text-slate-400">{s.namespace}</td>
                <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">{new Date(s.expiresAt).toLocaleDateString()}</td>
