...
export default function ConfigPage() {
  ...
  const yamlCommitMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/config/platform", {
        ...
        body: JSON.stringify({
          yamlContent,
          commitMessage: "chore: update platform.yaml via InfraWeaver Console raw editor",
        }),
      });
      if (!res.ok) throw new Error("YAML commit failed");
      return res.json();
    },
    onSuccess: () => toast.success("platform.yaml saved & committed to git!"),
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 bg-white/5 rounded-lg p-1 mb-6 overflow-x-auto scrollbar-none">
        {tabs.map(tab => (
          <button
            key={tab.id}
