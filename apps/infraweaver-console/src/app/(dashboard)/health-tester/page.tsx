...
export default function HealthTesterPage() {
  ...
  const statusColor = (r?: TestResult) => !r ? "text-slate-400" : r.error ? "text-red-400" : r.status && r.status < 400 ? "text-green-400" : "text-red-400";
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">Test HTTP endpoints and check their health</p>
        </div>
      ...
      <div className="bg-slate-900/60 border border-white/10 rounded-xl backdrop-blur-sm p-4">
        <h3 className="text-sm font-semibold text-white mb-3">Add Endpoint</h3>
        <div className="flex gap-2">
          <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Name (optional)" className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white placeholder:text-slate-500 outline-none focus:border-indigo-500/50 w-36" />
          <input value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="https://..." className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-white placeholder:text-slate-500 outline-none focus:border-indigo-500/50" />
      ...
      <div className="space-y-3">
        {endpoints.map(ep => (
          <div key={ep.id} className={cn("bg-slate-900/60 border rounded-xl backdrop-blur-sm p-4 flex items-center gap-4", ep.result?.error || (ep.result?.status && ep.result.status >= 400) ? "border-red-500/20" : ep.result?.status ? "border-green-500/20" : "border-white/10")}>
            ...
            <div className="flex gap-2">
              <button onClick={() => handleTestOne(ep.id)} disabled={ep.testing} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-slate-300 hover:text-white transition-colors disabled:opacity-50">
