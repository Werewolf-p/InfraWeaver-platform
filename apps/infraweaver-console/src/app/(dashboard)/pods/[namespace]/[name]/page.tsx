...
export default function PodDetailPage() {
  ...
  if (isLoading) {
    return <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-6 text-sm text-slate-500 dark:text-slate-400">Loading pod details...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      {activeTab === "overview" && (
        <div className="grid gap-6 xl:grid-cols-[1.4fr_1fr]">
          <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-4">
                ...
                <p className="mt-3 text-sm text-gray-900 dark:text-white">{data.nodeName ?? "—"}</p>
              </div>
              <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Pod IP</p>
                <p className="mt-3 font-mono text-sm text-gray-900 dark:text-white">{data.podIP ?? "—"}</p>
              </div>
              <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-4">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Created</p>
                <p className="mt-3 text-sm text-gray-900 dark:text-white">{formatDate(data.createdAt)}</p>
              </div>
            </div>

            <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60">
              <div className="border-b border-gray-200 dark:border-white/10 px-4 py-3">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Containers</h2>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-white/10 text-left text-slate-500">
                  ...
                  <tbody>
                    {data.containers.map((container) => (
                      <tr key={container.name} className="border-b border-gray-200 dark:border-white/5 align-top last:border-b-0">
                        ...
                        <td className="px-4 py-3 font-mono text-xs text-slate-700 dark:text-slate-300">{container.image ?? "—"}</td>
                        <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatResources(container.requests)}</td>
                        <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{formatResources(container.limits)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-4">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Labels</h2>
              <div className="mt-4 space-y-2">
                {Object.entries(data.labels).length === 0 ? (
                  ...
                  Object.entries(data.labels).map(([key, value]) => (
                    <div key={key} className="rounded-xl border border-gray-200 dark:border-white/5 bg-slate-100 dark:bg-slate-950/60 px-3 py-2 text-sm">
                      <p className="text-xs text-slate-500">{key}</p>
                      <p className="mt-1 break-all font-mono text-gray-900 dark:text-white">{value}</p>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-4">
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Primary container</h2>
              <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">{selectedContainerData?.name ?? data.containers[0]?.name ?? "—"}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="max-w-sm">
            <select
              ...
              className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900 px-3 py-2 text-sm text-gray-900 dark:text-white outline-none focus:border-indigo-500/50"
            >
              {data.containers.map((container) => (
      ...
      {activeTab === "terminal" && (
        <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-slate-100 dark:bg-slate-900/60 p-6">
          <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Open pod shell</h2>
          ...
          <div className="space-y-3">
            {(eventsData?.events ?? []).length === 0 ? (
              ...
              (eventsData?.events ?? []).map((event) => (
                <div key={`${event.name}-${event.lastTimestamp}`} className="rounded-xl border border-gray-200 dark:border-white/5 bg-slate-100 dark:bg-slate-950/60 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{event.reason ?? "Pod event"}</p>
                  ...
                  <p className="mt-3 text-sm text-slate-700 dark:text-slate-300">{event.message ?? "No message"}</p>
