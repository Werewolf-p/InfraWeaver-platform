...
export default function WebhookTesterPage() {
  ...
  const statusColor = (s?: number) => !s ? "text-slate-500 dark:text-slate-400" : s < 300 ? "text-green-400" : s < 500 ? "text-yellow-400" : "text-red-400";

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <PageHeader icon={Zap} title="Webhook Tester" />
      <div>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2"><Globe className="w-5 h-5 text-slate-500 dark:text-slate-400" />Webhook Tester</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400">Send HTTP requests and inspect responses</p>
      </div>
      ...
      {result && (
        <div className="bg-slate-100 dark:bg-slate-900/60 border border-gray-200 dark:border-white/10 rounded-xl backdrop-blur-sm p-4 space-y-4">
          ...
          {result.body && (
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Response Body</p>
              <pre className="bg-black/30 rounded-lg p-3 text-xs text-green-300 overflow-auto max-h-64">{result.body}</pre>
            </div>
          )}
          {result.headers && Object.keys(result.headers).length > 0 && (
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Response Headers</p>
