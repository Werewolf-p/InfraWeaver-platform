...
function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span className={cn("text-xs font-semibold px-2 py-0.5 rounded-full border", {
      ...
      "bg-slate-500/20 text-slate-500 dark:text-slate-400 border-slate-500/30": !["critical","high","warning","medium","info","low"].includes(s),
    })}>
      {severity}
    </span>
  );
...
function SectionCard({ children, delay = 0 }: { children: React.ReactNode; delay?: number }) {
  return (
    <motion.div
      ...
      className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-3 md:p-5"
    >
      {children}
    </motion.div>
  );
...
function OverviewCard({ card, delay }: { card: StatCard; delay: number }) {
  ...
  const valColor = card.status === "crit" ? "text-red-400" : card.status === "warn" ? "text-orange-400" : "text-gray-900 dark:text-white";
  return (
    <motion.div
    ...
    <div>
      ...
      <div className="relative rounded-xl overflow-hidden mb-6">
        ...
        <div className="relative flex items-start justify-between p-5 gap-4 flex-wrap">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex min-h-[44px] items-center gap-2 rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-white/5 px-4 text-sm text-slate-700 dark:text-slate-300 transition-colors hover:bg-gray-100 dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white disabled:opacity-50"
          >
      ...
      <div className="space-y-6">
              ...
              <div className="flex-1 space-y-2 w-full">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={i} className="p-3 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                ...
                <div className="p-3 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 text-xs text-slate-500 dark:text-slate-400">
                  <span className="font-semibold text-gray-900 dark:text-white">Total Score: {postureData.score}/100 · Grade {postureData.grade}</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900 dark:text-white font-medium truncate">{cert.name}</p>
              ...
              <div className="space-y-2">
                {(enhanced?.externalSecrets ?? []).map((es, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                    className={cn("flex items-center gap-3 p-3 rounded-lg border", es.ready ? "bg-gray-100 dark:bg-white/5 border-gray-200 dark:border-white/5" : "bg-red-500/5 border-red-500/20")}
                  >
                    <StatusDot ok={es.ready} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900 dark:text-white font-medium truncate">{es.name}</p>
                  ...
                  {["all", "high", "medium", "low"].map(sev => (
                    <button
                      ...
                      className={cn("min-h-[44px] rounded-full border px-4 py-2 text-sm font-semibold transition-colors whitespace-nowrap", kyvernoSevFilter === sev
                        ...
                        : "bg-gray-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10"
                      )}
                    >
              ...
              {kyvernoLoading ? <Shimmer rows={4} /> : filteredKyvernoViolations.length === 0 ? (
                ...
                <>
                  ...
                  {filteredKyvernoViolations.map((v, i) => (
                    <div key={`card-${i}`} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-white/5 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-mono text-base font-semibold text-slate-100">{v.policy}</p>
                          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{v.namespace} · {v.kind}</p>
                        </div>
                        <SeverityBadge severity={v.severity} />
                      </div>
                      <p className="mt-3 text-sm text-slate-700 dark:text-slate-300">{v.resource}</p>
                      <p className="mt-2 text-sm text-slate-500">{v.message}</p>
                      <button
                        type="button"
                        onClick={() => setExpandedKyverno(expandedKyverno === i ? null : i)}
                        className="mt-3 inline-flex min-h-[44px] items-center gap-2 rounded-2xl border border-gray-200 dark:border-white/10 px-4 text-sm text-slate-700 dark:text-slate-300"
                      >
                      ...
                      {expandedKyverno === i ? (
                        <div className="mt-3 rounded-2xl border border-gray-200 dark:border-white/10 bg-[#0f1115] p-4 text-sm text-slate-700 dark:text-slate-300">
                          ...
                          <p className="mt-3 text-indigo-300">Review the Kyverno policy <code className="rounded bg-gray-100 dark:bg-white/10 px-1 font-mono">{v.policy}</code> and update the resource spec to comply.</p>
                        </div>
                      ) : null}
                    </div>
                  ))}
                      ...
                      {filteredKyvernoViolations.map((v, i) => (
                        <>
                          <tr
                            key={`row-${i}`}
                            onClick={() => setExpandedKyverno(expandedKyverno === i ? null : i)}
                            className="cursor-pointer transition-colors hover:bg-gray-100 dark:hover:bg-white/5"
                          >
                            <td className="max-w-[120px] truncate px-1 py-2 font-mono text-slate-800 dark:text-slate-200">{v.policy}</td>
                            <td className="max-w-[100px] truncate px-1 py-2 text-slate-500 dark:text-slate-400">{v.resource}</td>
                            <td className="hidden px-1 py-2 text-slate-500 dark:text-slate-400 sm:table-cell">{v.namespace}</td>
                          ...
                          {expandedKyverno === i && (
                            <tr key={`exp-${i}`}>
                              <td colSpan={5} className="px-1 pb-2">
                                <div className="space-y-1 rounded-lg border border-gray-200 dark:border-white/5 bg-gray-100 dark:bg-white/5 p-3 text-sm text-slate-500 dark:text-slate-400">
                                  <p><span className="text-slate-500">Category:</span> {v.category}</p>
                                  <p><span className="text-slate-500">Kind:</span> {v.kind}</p>
                                  <p><span className="text-slate-500">Message:</span> {v.message}</p>
                                  <p className="mt-2 text-indigo-400">💡 Remediation: Review the Kyverno policy <code className="rounded bg-gray-100 dark:bg-white/10 px-1 font-mono">{v.policy}</code> and update the resource spec to comply.</p>
                                </div>
                              </td>
                            </tr>
                          )}
          ...
          >
            {(podsLoading || enhancedLoading) ? <Shimmer rows={4} /> : (() => {
              ...
              return totalIssues === 0 ? (
                <div className="text-center py-8">
                  <CheckCircle2 className="w-10 h-10 text-green-400 mx-auto mb-2" />
                  <p className="text-sm text-slate-500 dark:text-slate-400">No pod security issues detected</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {/* Severity bars */}
                  <div className="grid grid-cols-4 gap-2 mb-2">
                    {[
                      ...
                      <div key={label} className="p-2 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 text-center">
                  ...
                  {issues.length > 0 ? (
                    Array.from(issuesByNs.entries()).map(([ns, nsIssues]) => (
                      <div key={ns}>
                        <button
                          onClick={() => toggleNs(ns)}
                          className="w-full flex items-center gap-2 py-1.5 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition-colors"
                        >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <ul className="mt-1 space-y-0.5">
                                    {issue.issues.map((iss, j) => (
                                      <li key={j} className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                  ...
                  {filteredAuthEvents.map((evt, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-gray-900 dark:text-white font-medium truncate">{evt.user}</p>
            ...
            <div className="space-y-3">
              <div className="space-y-3 sm:hidden">
                {(rbacData?.serviceAccounts ?? []).filter(sa => sa.bindings.length > 0).map((sa, i) => (
                  <div key={i} className={cn("rounded-2xl border p-4", (sa.isClusterAdmin || sa.hasWildcard) ? "border-red-500/20 bg-red-500/5" : "border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-white/5")}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-base font-semibold text-gray-900 dark:text-white">{sa.name}</p>
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{sa.namespace}</p>
                      </div>
                    ...
                    <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{sa.bindings.join(", ")}</p>
                  </div>
                ))}
              </div>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {(rbacData?.serviceAccounts ?? []).filter(sa => sa.bindings.length > 0).map((sa, i) => (
                      <tr key={i} className={cn("transition-colors", (sa.isClusterAdmin || sa.hasWildcard) && "bg-red-500/5")}>
                        <td className="py-2 font-medium text-slate-800 dark:text-slate-200">{sa.name}</td>
                        <td className="py-2 text-slate-500 dark:text-slate-400">{sa.namespace}</td>
                        <td className="max-w-xs truncate py-2 text-slate-500 dark:text-slate-400">{sa.bindings.join(", ")}</td>
                        <td className="py-2">
                          {sa.isClusterAdmin && <SeverityBadge severity="Critical" />}
                          {!sa.isClusterAdmin && sa.hasWildcard && <SeverityBadge severity="Warning" />}
                        </td>
            ...
            <div className="space-y-3">
              <div className="space-y-3 sm:hidden">
                {(enhanced?.pdbList ?? []).map((pdb, i) => (
                  <div key={i} className={cn("rounded-2xl border p-4", pdb.disruptionsAllowed === 0 ? "border-orange-500/20 bg-orange-500/5" : "border-gray-200 dark:border-white/10 bg-gray-100 dark:bg-white/5")}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-base font-semibold text-gray-900 dark:text-white">{pdb.name}</p>
                        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{pdb.namespace}</p>
                      </div>
                    ...
                    <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        <dd className="mt-1 text-gray-900 dark:text-white">{pdb.minAvailable !== undefined ? `min:${pdb.minAvailable}` : pdb.maxUnavailable !== undefined ? `max:${pdb.maxUnavailable}` : "—"}</dd>
                      </div>
                      <div>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {(enhanced?.pdbList ?? []).map((pdb, i) => (
                      <tr key={i} className={cn("transition-colors", pdb.disruptionsAllowed === 0 && "bg-orange-500/5")}>
                        <td className="py-2 font-medium text-slate-800 dark:text-slate-200">{pdb.name}</td>
                        <td className="py-2 text-slate-500 dark:text-slate-400">{pdb.namespace}</td>
            ...
            {enhancedLoading ? <Shimmer rows={3} /> : (
              <div className="space-y-2">
                {(enhanced?.unprotectedNamespaces ?? []).length === 0 ? (
                  <div className="text-center py-8">
                    <CheckCircle2 className="w-10 h-10 text-green-400 mx-auto mb-2" />
                    <p className="text-sm text-slate-500 dark:text-slate-400">All namespaces have NetworkPolicies</p>
                  </div>
                ) : (
                  <>
                    ...
                    {(enhanced?.unprotectedNamespaces ?? []).map((ns, i) => (
                      ...
                      >
                        ...
                        <span className="text-sm text-gray-900 dark:text-white font-medium">{ns}</span>
          ...
          {enhancedLoading ? <Shimmer rows={2} /> : enhanced?.openbaoStatus ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className={cn("p-3 rounded-lg border", item.ok ? "bg-gray-100 dark:bg-white/5 border-gray-200 dark:border-white/5" : "bg-red-500/5 border-red-500/20")}>
                  <p className="text-xs text-slate-500">{item.label}</p>
                  <p className={cn("text-sm font-semibold mt-0.5", item.ok ? "text-gray-900 dark:text-white" : "text-red-400")}>{String(item.value)}</p>
                </div>
              ))}
            </div>
          ) : (
          ...
          {enhancedLoading ? <Shimmer rows={3} /> : (
            <div className="space-y-2">
              {(enhanced?.nodePressure ?? []).map((node, i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                    className={cn("flex items-center gap-3 p-3 rounded-lg border", anyPressure ? "bg-red-500/5 border-red-500/20" : "bg-gray-100 dark:bg-white/5 border-gray-200 dark:border-white/5")}
                  >
                    <StatusDot ok={node.ready && !anyPressure} />
                    <div className="flex-1">
                      <p className="text-sm text-gray-900 dark:text-white font-medium">{node.name}</p>
                      {anyPressure ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.34, duration: 0.35 }}>
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                <StatusDot ok={true} />
                <div className="flex-1">
                  <p className="text-sm text-gray-900 dark:text-white font-medium">HMAC API Secret</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Dual-secret rotation supported — set <code className="font-mono bg-gray-100 dark:bg-white/10 px-1 rounded">CONSOLE_API_SECRET_PREV</code> during rollover.
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                <StatusDot ok={true} />
                <div className="flex-1">
                  <p className="text-sm text-gray-900 dark:text-white font-medium">NEXTAUTH_SECRET</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">NextAuth session encryption key — stored in Kubernetes ExternalSecret, sourced from OpenBao vault.</p>
                </div>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold flex-shrink-0">Vault-managed</span>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5">
                <StatusDot ok={true} />
                <div className="flex-1">
                  <p className="text-sm text-gray-900 dark:text-white font-medium">Authentik OIDC Client Secret</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">OAuth2 client secret for Authentik → NextAuth integration. Rotation requires redeployment of console.</p>
                </div>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold flex-shrink-0">Vault-managed</span>
              </div>
              <div className="mt-2 p-3 rounded-lg bg-blue-500/5 border border-blue-500/20">
                <p className="text-xs text-blue-300 flex items-start gap-2">
                  ...
                  <span>
                    To rotate the API HMAC secret: set <code className="font-mono bg-gray-100 dark:bg-white/10 px-1 rounded">CONSOLE_API_SECRET_PREV</code> = old value,
                    update <code className="font-mono bg-gray-100 dark:bg-white/10 px-1 rounded">CONSOLE_API_SECRET</code> = new value, then clear PREV after all pods restart.
                  </span>
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <SectionCard delay={0.28}>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Healthy</span><span className="text-green-400 font-semibold">{enhanced.overview.longhornHealthy}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Degraded</span><span className={cn("font-semibold", enhanced.overview.longhornDegraded > 0 ? "text-orange-400" : "text-slate-500")}>{enhanced.overview.longhornDegraded}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Faulted</span><span className={cn("font-semibold", enhanced.overview.longhornFaulted > 0 ? "text-red-400" : "text-slate-500")}>{enhanced.overview.longhornFaulted}</span></div>
              </div>
            </SectionCard>
            <SectionCard delay={0.3}>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">OutOfSync</span><span className={cn("font-semibold", enhanced.overview.argocdOutOfSync > 0 ? "text-orange-400" : "text-green-400")}>{enhanced.overview.argocdOutOfSync}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">cert-manager</span><span className="text-gray-900 dark:text-white font-semibold">{enhanced.overview.certCount} certs</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Renewal pending</span><span className={cn("font-semibold", enhanced.overview.certRenewalPending > 0 ? "text-orange-400" : "text-green-400")}>{enhanced.overview.certRenewalPending}</span></div>
              </div>
            </SectionCard>
            <SectionCard delay={0.32}>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">IPs in use</span><span className="text-gray-900 dark:text-white font-semibold">{enhanced.overview.metallbPoolUsed}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Pool total</span><span className="text-gray-900 dark:text-white font-semibold">{enhanced.overview.metallbPoolTotal}</span></div>
                <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Utilization</span>
