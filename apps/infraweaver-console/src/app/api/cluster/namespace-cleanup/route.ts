  ...
  try {
    ...
    for (const pod of pods.items) {
      ...
      if (reason === "Evicted" || phase === "Failed") {
        try { await coreApi.deleteNamespacedPod({ name, namespace }); deleted.push(`pod/${name}`); } catch (e) { errors.push(`pod/${name}: ${safeError(e)}`); }
      } else if (isTerminating) {
        try {
          await coreApi.patchNamespacedPod({ name, namespace, body: { metadata: { finalizers: [] } } });
          deleted.push(`pod/${name} (finalizers removed)`);
        } catch (e) { errors.push(`pod/${name}: ${safeError(e)}`); }
      }
    }
    ...
    for (const job of jobs.items) {
      ...
      if (active === 0 && (succeeded > 0 || failed > 0)) {
        try { await batchApi.deleteNamespacedJob({ name, namespace }); deleted.push(`job/${name}`); } catch (e) { errors.push(`job/${name}: ${safeError(e)}`); }
      }
    }
