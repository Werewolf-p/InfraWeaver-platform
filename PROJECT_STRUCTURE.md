...
GitHub Actions configuration, automation hooks, and the AI agent knowledge base.
...
|---|---|
| `deploy.yml` | Full cluster deploy pipeline. Runs Terraform → seeds secrets → applies ArgoCD bootstrap. Triggered on push to `main`. |
| `update-console.yml` | Builds and pushes `infraweaver-console` Docker image, then patches the K8s deployment. |
| `update-api.yml` | Builds and pushes `infraweaver-api` Docker image, then patches the K8s deployment. |
| `validate.yml` | Linting: yamllint, shellcheck, terraform validate, actionlint. Runs on every PR. |
| `maintenance.yml` | Scheduled jobs: TLS backup refresh, etcd snapshot, cert expiry checks. |
| `pr-labeler.yml` | Auto-labels PRs based on paths changed (uses `.github/labeler.yml`). |
...
| Action | What it does |
|---|---|
| `load-env` | Reads `platform.yaml` and exports env vars used by downstream steps. |
| `setup-kubectl` | Configures `kubectl` with the cluster kubeconfig from GitHub secrets. |
...
| `modules/github-runner/` | Provisions the self-hosted GitHub Actions runner VM |
...
| `playbooks/github-runner.yml` | Provisions the GitHub Actions runner VM with required tools |
...
Run in order during a fresh cluster deploy. Called by the `deploy.yml` GitHub Actions workflow.

| Script | What it does |
|---|---|
| `install-tools.sh` | Installs kubectl, talosctl, argocd, helm onto the runner |
...
| `setup-runner-env.sh` | Installs tools on a new GitHub Actions runner VM |
